...
class MailChimpImport {
  ...
  // params:
  //  - site
  //  - campaignId (if MailChimp campaign id is provided, only fetch the report data of that id, site param will be ignored)
  //
  async fetchReportData(site, campaignId) {
    let reportData = [];
    let docs = typeof campaignId === "undefined" ?
                await this.getAllCampaignDbDatabySite(site, { _id: 1, type: 1, variate_settings: 1, year: 1, quarter: 1, month: 1, promo_num: 1, segment: 1 }) :
                await this.getCampaignDbData({ _id: campaignId }, { _id: 1, type: 1, variate_settings: 1, year: 1, quarter: 1, month: 1, promo_num: 1, segment: 1 });
