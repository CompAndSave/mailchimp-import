...
class MailChimpImport {
  ...
  async fetchReportData(site, startTime, campaignId) {
    ...
    let docs = typeof campaignId === "string" ? await this.getCampaignDbData({ _id: campaignId }, projection) :
               typeof startTime === "string" ? await this.getCampaignDbData({ list_id: this.audienceId[site], year: { $gte: Number(startTime.substring(0, 4)) }, month: { $gte: Number(startTime.substring(5, 7)) }}, projection) :
    ...
    for (let i = 0; i < docs.length; ++i) {  
      if (docs[i].type === "variate") {
        ...
        reportData.push(formatCampaignReportData(docs[i], mcReportData[i], {
          type: "variate-parent",
          child_ids: childIds
        }));
